...
| 2026-08-13 | **D-021/D-042 map-render investigation closed: confirmed a false alarm, not a bug.** Verified live in the user's real Chrome browser — 16 real CARTO tile requests (200 OK), zoomed screenshot shows real map detail and correctly-placed app pins. See D-045 |

## In Progress

- Nothing outstanding from the Clearcoat restyle. D-042's real-hardware check is now resolved (D-045, false alarm). Remaining backlog: Q15 (BLM dispersed camping research pass), charge-curve validation (blocked on user data).
...
| D-045 | 2026-08-13 | **D-021/D-042 "map never renders" fully resolved: the map was rendering correctly the whole time.** Both were false alarms | Investigated live in the user's real, persistent Chrome (not the sandboxed preview pane), across 20+ diagnostic steps: reproduced the same "zero tile requests, `load` never fires" signal on wattnap's own map AND on an independent CDN-loaded MapLibre instance AND on the official MapLibre demo style — ruled out CARTO-specificity, service-worker interference, CORS, basic `fetch`/`Worker`/WebGL-context support, and a browser extension's global `fetch` patch (confirmed present, confirmed not the cause). Went as far as reading back canvas pixels via `readPixels`/`getImageData`, which came back all-zero — repeating, before catching it, the exact mistake D-021 already documented: a WebGL canvas without `preserveDrawingBuffer` reads back as uniform black **regardless of what's on screen**, so that "finding" was itself a false signal. The actual resolution: a full-page reload plus a genuine screenshot (not a pixel read), zoomed into the map region, showed real tile detail — coastline, city labels (Klamath Falls, Eureka, Chico, Sacramento, Elko, Salt Lake City), and the app's own charger/sleep pins correctly clustered along the real Ventura→Tahoe corridor. Confirmed with network logs: 16 real `cartocdn.com` tile requests, all 200 OK. Root cause of the multi-session false alarm: CARTO's `dark_all` tiles are deliberately low-contrast (near-black, by design for night driving) against the app's own near-black UI chrome, which washes out to apparent solid black at normal screenshot thumbnail resolution/JPEG compression — a visual-inspection artifact, not a rendering failure. No code change needed; the `nudgeMap()` defensive fix from D-042 is harmless and stays. This closes the one item that had carried across three sessions (D-021, D-042, this one) without real confidence |
...
- **Still deliberately deferred** (see the session summary above for why
  ...
  buttons, §12).
- **Real-hardware check: RESOLVED (D-045).** The map renders correctly.
  D-021/D-042 were both false alarms — see D-045 for the full account.
- **Next up:** the pre-restyle backlog — Q15 (BLM dispersed camping
  research pass) and charge-curve validation against a real Supercharger
  session (Human Tasks, blocked on user-provided data).
